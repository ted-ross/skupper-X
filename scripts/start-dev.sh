...
for arg in "$@"; do
    case $arg in
        ...
        --help|-h)
            ...
            echo ""
            echo "This script starts Skupper-X in development mode with:"
            echo "  - Backend (Management Controller) with hot reload on port 8085"
            echo "  - Frontend (Console) with hot reload on port 3000"
            echo "  - PostgreSQL database in Docker container"
            echo ""
            echo "To update outdated packages, run:"
            echo "  ./scripts/update-packages.sh"
            exit 0
            ;;
        *)
            # Unknown option
...
# Funzione para verificar paquetes desactualizados
check_outdated_packages() {
    ...
    # Check if npm outdated shows any outdated packages (non-interactive)
    ...
    if [ -n "$outdated_output" ]; then
        ...
        log_info "  ./scripts/update-packages.sh"
        log_info "Or to skip this check: ./scripts/start-dev-mode.sh --skip-package-check"
        echo ""
    else
...
# Funzione para verificar y instalar nodemon si es necesario
ensure_nodemon() {
    cd "$MANAGEMENT_CONTROLLER_DIR"
    
    # Check if nodemon is installed as dev dependency
    if ! npm list nodemon --depth=0 >/dev/null 2>&1; then
        log_info "Installing nodemon for hot reload functionality..."
        npm install --save-dev nodemon
        log_success "Nodemon installed successfully"
    fi
}
...
# Check for outdated packages in both components (unless skipped)
...
# Ensure nodemon is available for hot reload
ensure_nodemon
...
# 4. Avvia il Management Controller (Backend) with hot reload
