...
# U    # Build console (React app)
    log_info "Building console (React app)..."
    cd "$CONSOLE_DIR"
    
    # Verifica e installa dipendenze console se necessario
    if [ ! -d "node_modules" ]; then
        ...
        if [ "package.json" -nt "node_modules" ] || [ "yarn.lock" -nt "node_modules" ]; then
            log_info "Console dependencies are outdated, updating..."
            if ! yarn install >/dev/null 2>&1; then
                log_error "Failed to update console dependencies"
                return 1
            fi
        else
            log_success "Console dependencies are up to date"
        fi
    fi
    ...
    log_info "Checking management controller dependencies..."
    
    # Verifica se package.json esiste e se node_modules è aggiornato
    if [ -f "package.json" ]; then
        if [ ! -d "node_modules" ]; then
            log_info "Installing management controller dependencies..."
            if ! npm install >/dev/null 2>&1; then
                log_error "Failed to install management controller dependencies"
                return 1
            fi
        else
            # Verifica se package.json è più recente di node_modules
            if [ "package.json" -nt "node_modules" ] || [ -f "package-lock.json" -a "package-lock.json" -nt "node_modules" ]; then
                log_info "Management controller dependencies are outdated, updating..."
                if ! npm install >/dev/null 2>&1; then
                    log_error "Failed to update management controller dependencies"
                    return 1
                fi
            else
                log_success "Management controller dependencies are up to date"
            fi
        fi
    fi
    
    # Build management controller (includes console integration)
    log_info "Building management controller with integrated console..." Docker
