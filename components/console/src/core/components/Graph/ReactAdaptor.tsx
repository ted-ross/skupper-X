...
const GraphReactAdaptor: FC<GraphReactAdaptorProps> = memo(
  forwardRef(
    (
      ...
      return (
        <div ref={graphRef} style={{ height: '90%', background: GRAPH_BG_COLOR, position: 'relative' }}>
          ...
          <div
            style={{
              zIndex: 9999,
              position: 'absolute',
              left: `${menuContext.x || -10000}px`,
              top: `${menuContext.y}px`
            }}
          >
            {ContextMenuComponent && menuContext.item && (
              <ContextMenuComponent item={menuContext.item} target={menuContext.target} />
            )}
          </div>
        </div>
      );
