  ...
  try {
    ...
    try {
      ...
      const result = await client.query(
        "INSERT INTO Backbones(Name, LifeCycle, MultiTenant) VALUES ($1, 'partial', $2) RETURNING Id",
        [norm.name, norm.multitenant]
      );
...
const createBackboneSite = async function (req, res) {
  ...
  try {
    ...
    try {
      ...
      //
      const result = await client.query(
        `INSERT INTO InteriorSites(Name, TargetPlatform, Backbone${extraCols}) VALUES ($1, $2, $3${extraVals}) RETURNING Id`,
  ...
  try {
    ...
    try {
      ...
      const siteResult = await client.query(
        "SELECT * FROM InteriorSites WHERE Id = $1",
        [sid]
      );
      if (siteResult.rowCount == 1) {
        ...
        if (norm.name != null && norm.name != site.name) {
          nameChanged = true;
          await client.query(
            "UPDATE InteriorSites SET Name = $1 WHERE Id = $2",
            [norm.name, sid]
          );
          siteName = norm.name;
        }
        ...
        if (norm.metadata != null && norm.metadata != site.metadata) {
          await client.query(
            "UPDATE InteriorSites SET Metadata = $1 WHERE Id = $2",
            [norm.metadata, sid]
          );
        }
      }
...
const createAccessPoint = async function (req, res) {
  ...
  try {
    ...
    try {
      ...
      const siteResult = await client.query(
        "SELECT Name from InteriorSites WHERE Id = $1",
        [sid]
      );
      ...
      //
      const result = await client.query(
        `INSERT INTO BackboneAccessPoints(Name, Kind, InteriorSite${extraCols}) VALUES ($1, $2, $3${extraVals}) RETURNING Id`,
        [name, norm.kind, sid]
      );
...
const createBackboneLink = async function (req, res) {
  ...
  try {
    ...
    try {
      ...
      if (siteResult.rowCount == 0) {
        throw Error(
          `Referenced connecting site not found: ${norm.connectingsite}`
        );
      }
      ...
      const linkResult = await client.query(
        "INSERT INTO InterRouterLinks(AccessPoint, ConnectingInteriorSite, Cost) VALUES ($1, $2, $3) RETURNING Id",
        [apid, norm.connectingsite, norm.cost]
      );
      ...
      try {
        ...
        Log(
          `Exception createBackboneLink module notifications: ${error.message}`
        );
        Log(error.stack);
      }
    } catch (error) {
    ...
    try {
      ...
      if (linkResult.rowCount == 1) {
        ...
        if (norm.cost != null && norm.cost != link.cost) {
          await client.query(
            "UPDATE InterRouterLinks SET Cost = $1 WHERE Id = $2",
            [norm.cost, lid]
          );
          returnStatus = 200;
...
const activateBackbone = async function (req, res) {
  ...
  try {
    ...
    await client.query(
      "UPDATE Backbones SET Lifecycle = 'new' WHERE Id = $1 and LifeCycle = 'partial'",
      [bid]
    );
    await client.query("COMMIT");
    res.status(returnStatus).end();
...
const deleteBackbone = async function (req, res) {
  ...
  try {
    ...
    const vanResult = await client.query(
      "SELECT Id FROM ApplicationNetworks WHERE Backbone = $1 and LifeCycle = 'ready' LIMIT 1",
      [bid]
    );
    ...
    const siteResult = await client.query(
      "SELECT Id FROM InteriorSites WHERE Backbone = $1 LIMIT 1",
      [bid]
    );
    if (siteResult.rowCount > 0) {
  ...
  try {
    ...
    if (connectingSite) {
      try {
        ...
        Log(
          `Exception deleteBackboneLink module notifications: ${error.message}`
        );
        Log(error.stack);
      }
    }
  ...
  try {
    ...
    if (bid) {
      ...
      result = await client.query(
        "SELECT Id, Name, Lifecycle, Failure, MultiTenant FROM Backbones WHERE Id = $1",
        [bid]
      );
    } else {
      result = await client.query(
        "SELECT Id, Name, Lifecycle, Failure, MultiTenant FROM Backbones"
      );
    }
...
const listBackboneSites = async function (req, res) {
  ...
  try {
    ...
    const result = await client.query(
      "SELECT InteriorSites.Id, Name, Lifecycle, Failure, Metadata, DeploymentState, TargetPlatform, FirstActiveTime, LastHeartbeat, " +
...
const listBackboneLinksForSite = async function (req, res) {
  ...
  try {
    ...
    // Get outgoing links: find links where the access point belongs to this site
    const result = await client.query(
      "SELECT InterRouterLinks.Id, InterRouterLinks.Cost, " +
        "BackboneAccessPoints.Name as accesspoint, " +
        "ConnectingSites.Name as connectinginteriorsite " +
        "FROM InterRouterLinks " +
        "JOIN BackboneAccessPoints ON BackboneAccessPoints.Id = InterRouterLinks.AccessPoint " +
        "JOIN InteriorSites AS ConnectingSites ON ConnectingSites.Id = InterRouterLinks.ConnectingInteriorSite " +
    ...
    result.rows.forEach((row) => {
      list.push({
        ...
        connectinginteriorsite: row.connectinginteriorsite,
        cost: row.cost,
      });
    });
    res.json(list);
...
const listInvitations = async function (res) {
  ...
  const client = await db.ClientFromPool();
  const result = await client.query(
    "SELECT Id, Name, Lifecycle, Failure FROM MemberInvitations"
  );
  var list = [];
  result.rows.forEach((row) => {
    list.push(row);
...
exports.Initialize = async function (app, keycloak) {
  ...
  app
    .route(API_PREFIX + "backbones", keycloak.protect("realm:backbone-admin"))
    .post(createBackbone) // CREATE
    .get(listBackbones); // LIST

  app
    .route(
      API_PREFIX + "backbones/:bid",
      keycloak.protect("realm:backbone-admin")
    )
  ...
  app
    .route(
      API_PREFIX + "backbones/:bid/activate",
      keycloak.protect("realm:backbone-admin")
    )
  ...
  app
    .route(
      API_PREFIX + "backbones/:bid/sites",
      keycloak.protect("realm:backbone-admin")
    )
  ...
  app
    .route(
      API_PREFIX + "backbonesites/:sid/accesspoints",
      keycloak.protect("realm:backbone-admin")
    )
  ...
  app
    .route(
      API_PREFIX + "backbones/:bid/accesspoints",
      keycloak.protect("realm:backbone-admin")
    )
  ...
  app
    .route(
      API_PREFIX + "accesspoints/:apid/links",
      keycloak.protect("realm:backbone-admin")
    )
  ...
  app
    .route(
      API_PREFIX + "backbones/:bid/links",
      keycloak.protect("realm:backbone-admin")
    )
  ...
  app
    .route(
      API_PREFIX + "backbonesites/:sid/links",
      keycloak.protect("realm:backbone-admin")
    )
